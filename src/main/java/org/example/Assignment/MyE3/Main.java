    ...
    public static <T> T inputInfor(String prompt, Supplier<T> supplier, Predicate<T> validator, String errorMessage) {
        while (true) {
            try {
                ...
                if(input.equals(-1) || input.equals(-1.0)) return null; // Kiểu truyền vào buộc phải là các wrapper classz
    ...
    public static Integer getEditChoice(Scanner sc, Student s){
        ...
        while(true){
            try{
                ...
                if(choice<1 || choice>3) throw new IllegalArgumentException("Invalid choice ! Choice must be 1,2 or 3.");
                return choice;
            } catch (Exception e) {
                System.out.println("Error : "+e.getMessage()+"\n");
            }
                            ...
                            while(true){
                                try{
                                    ...
                                    switch (choice){
                                        ...
                                        default -> System.out.println("Error : Unknown error. Please find it by yourself hehe");
                                    }
                                    System.out.println("=============================================================");
                                } catch (Exception e) {
